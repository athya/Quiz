...
//mostly functional. Need to implement proper radio button checking (others unclick when one is clicked)
...
const questionList = [
  new Question('Who is the main character of the movie "Tangled"?', ['Rapunzel', 'Merida', 'Aurora'], 'Rapunzel', 'MC'),
  new Question('Which of the following songs was in "High School Musical"?', ['"Step it up"', '"Get your Head in the Game"'], '"Get your Head in the Game"', 'MC'),
  new Question('What movie features some rather adorable dogs?', ['Lady and the Tramp', '101 Dalmations','Both of the above!'], 'Both of the above!', 'MC')
];
...
function displayMCChoice() {
  ...
  for (let i = 0; i < questionList[count].answers.length; i++) {
    ...
    radio.setAttribute('name', questionList[count].answers[i]);
...
function displayResult() {
  ...
  qText.textContent =
    'You got ' + numRight + ' out of ' + numQ + ' right!';
}